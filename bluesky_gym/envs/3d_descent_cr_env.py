...
class DescentEnvCR3D(gym.Env):
    ...
    def __init__(self, render_mode=None, scenario_path=None):
        ...
        if scenario_path is not None:
            ...
            if path.is_dir():
                # If directory, get all .scn files
                ...
                # Use first scenario to initialize observation space
                init_scenario = self.scenario_files[0]
            elif path.is_file():
                # Single file - use it for all episodes
            ...
            # Parse initial scenario to determine observation space
        ...
        # Store current scenario path (will be updated in reset)
    ...
    def _get_obs(self):
        ...
        # # Pad intruder arrays to fixed length self.num_intruders
        # while len(self.intruder_distance) < self.num_intruders:
    ...
    # Reset
    # =========================
    def _generate_intruders(self, acid = 'KL001'):
    ...
    def reset(self, seed=None, options=None):
        ...
        # Randomly select a scenario if multiple are available
        if self.scenario_files:
            self.current_scenario = random.choice(self.scenario_files)
            # Update lat/long based on selected scenario
